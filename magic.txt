...
txt_to_encode = r'C:\temp\document-small.txt'
...
def substitution_encrypt(text, key_lower, key_upper): # case sensitive
    ...
    lower_table = str.maketrans(string.ascii_lowercase, key_lower)
    upper_table = str.maketrans(string.ascii_uppercase, key_upper)
    result = ''
    for char in text:
        if char.islower():
            result += char.translate(lower_table)
        elif char.isupper():
            result += char.translate(upper_table)
        else:
...
if not generate_video_only:
    ...
    print('Number of frames     = {}'.format(len(msg) / max_qr_msg_length))
    ...
    msg2 = msg1.replace(' ', '>')
    if (encrypt_message):
        msg2 = substitution_encrypt(msg2, 'dummy', 'dummy')
